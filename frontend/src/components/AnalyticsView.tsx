...
function ValidationCard({title,value}:{title:string;value:ValidationMetrics}) {
  if(!value.available) return <section className="analytics-card validation-card"><div><span className="section-kicker">VALIDATION</span><h3>{title}</h3><p>Run the evaluation script to generate this report.</p></div></section>;
  return <section className="analytics-card validation-card"><div><span className="section-kicker">OFFLINE SYNTHETIC VALIDATION</span><h3>{title}</h3><p>{value.email_count} emails · frozen evaluation artifact</p></div><div className="validation-grid"><div><span>Classifier only</span><strong>{percent(value.model_accuracy)}</strong></div><div><span>Rules + model</span><strong>{percent(value.pipeline_accuracy)}</strong></div><div><span>Field F1</span><strong>{percent(value.field_f1)}</strong></div><div><span>End-to-end</span><strong>{percent(value.end_to_end)}</strong></div></div><p className="validation-note">Classifier only excludes business rules. Synthetic test performance only; it is not a production-accuracy claim.</p></section>;
}
...
export default function AnalyticsView() {
  ...
  return <section className="analytics-page"><div className="analytics-heading"><div><span className="section-kicker">OPERATIONS ANALYTICS</span><h2>Inbox quality and automation</h2><p>Live saved reports, plus clearly labelled offline validation results.</p></div><span className={`analytics-ai ${data.ai_assistant.enabled?'active':''}`}>{data.ai_assistant.enabled?`Gemini active · ${data.ai_assistant.requests_used}/${data.ai_assistant.request_limit} calls`:'Gemini fallback off'}</span></div>
    ...
    <div className="analytics-grid"><section className="analytics-card"><span className="section-kicker">CLASSIFICATION</span><h3>Email categories</h3><BarList items={categoryItems} colors={['#0b5cab']}/></section><section className="analytics-card"><span className="section-kicker">DOCUMENT CHECKS</span><h3>Mismatches by field</h3><BarList items={fieldItems} colors={['#c33b47']}/></section><section className="analytics-card"><span className="section-kicker">RELIABILITY</span><h3>Human review reasons</h3><BarList items={reasonItems} colors={['#c98212']}/></section></div>
