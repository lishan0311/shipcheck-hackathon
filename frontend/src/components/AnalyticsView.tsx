...
function ReliabilityCard({items}:{items:{label:string;value:number;note?:string}[]}) {
  ...
  return <section className="analytics-card"><span className="section-kicker">RELIABILITY</span><h3>Human review reasons</h3>{total ? <BarList items={items} colors={items.map(() => '#0b5cab')}/> : <div className="reliability-empty"><strong>No reliability exceptions</strong><p>None of the current saved reports required review for wrong document type, missing attachment, unreadable input, or missing value.</p></div>}</section>;
}
