...
export const api = {
  ...
  archived: () => request<Inbox>('/archived'),
  archive: (emailIds: string[]) => request<{updated: number}>('/emails/archive', {method: 'POST', body: JSON.stringify({email_ids: emailIds})}),
  restore: (emailIds: string[]) => request<{updated: number}>('/emails/restore', {method: 'POST', body: JSON.stringify({email_ids: emailIds})}),
