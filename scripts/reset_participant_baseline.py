"""Restore the supplied 520-email dataset to one automated report per email.
...
def main() -> None:
    parser = argparse.ArgumentParser(description='Restore the participant inbox automated baseline.')
    parser.add_argument('--apply', action='store_true', help='Delete later report versions and reviews.')
    ...
    try:
        ...
        if len(grouped) != 520 or any(not values for values in grouped.values()):
        ...
        print(f'Automated baseline reports kept: {len(kept)}')
        print(f'Later report versions selected for deletion: {len(remove)}')
        ...
        if not args.apply:
            print('Dry run only. Re-run with --apply to restore the automated baseline.')
            return
        ...
        print('Baseline restored. Original emails and attachments were not changed.')
    finally:
        repo.close()
